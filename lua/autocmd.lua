...
G.api.nvim_create_autocmd(
	{ "BufEnter" },
	{ command = [[if &buftype == '' && &readonly == 1 | set buftype=acwrite | set noreadonly | endif]] }
)
G.api.nvim_create_autocmd(
	{ "BufReadPost" },
...
local function _javascript()
	G.map({
		...
		},
		{
			"v",
			"T",
			':<c-u>call SurroundVaddPairs("try {", "} catch (e) {}")<cr>',
			{ noremap = true, silent = true, buffer = true },
		},
	})
end
...
local function _typescript()
	G.map({
		...
		},
		{
			"v",
			"T",
			':<c-u>call SurroundVaddPairs("try {", "} catch (e: any) {}")<cr>',
			{ noremap = true, silent = true, buffer = true },
		},
	})
end
...
local function _markdown()
	G.hi({
		["MDTodoDate"] = { fg = "green", italic = false },
		["MDDoneDate"] = { fg = "grey", italic = true, strikethrough = true },
		["MDTodoText"] = { fg = "white", italic = false },
		["MDDoneText"] = { fg = "green", italic = true, strikethrough = true },
		["MDDeadline"] = { fg = "red", bold = true, underline = true },
		["MDNearline"] = { fg = "pink", bold = true },
	})
	G.cmd([[
	...
	G.map({
		{
			"n",
			"<cr>",
			":call v:lua.G_markdown_toggleCheck(0)<cr><cr>",
			{ noremap = true, silent = true, buffer = true },
		},
		{
			"n",
			"<2-LeftMouse>",
			":call v:lua.G_markdown_toggleCheck(1)<cr><2-LeftMouse>",
			{ noremap = true, silent = true, buffer = true },
		},
		...
		-- { "v", "`", ':<c-u>call SurroundVaddPairs("`", "``")<cr>', { noremap = true, silent = true, buffer = true } },
		{
			"v",
			"C",
			':<c-u>call SurroundVaddPairs("```plaintext", "```")<cr>',
			{ noremap = true, silent = true, buffer = true },
		},
		{ "n", "<F6>", ":call v:lua.G_markdown_toggleMPTheme()<cr>", { noremap = true, silent = true, buffer = true } },
		{
			"i",
			"<F6>",
			"<esc>:call v:lua.G_markdown_toggleMPTheme()<cr>",
			{ noremap = true, silent = true, buffer = true },
		},
	})
